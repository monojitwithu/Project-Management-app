...
const Controller = () => {
  ...
  const setProjectClickHandler = (Id) => {
    ...
    if (pojectStatus === 0) {
      const nob = [
        ...devolperData,
        (devolperData[positionOfTheUser].projectStatus = 1),
      ];
      setDevoloperData(nob);
    } else {
      const nob = [
        ...devolperData,
        (devolperData[positionOfTheUser].projectStatus = 0),
      ];
      setDevoloperData(nob);
    }
  };
  ...
  const endProjectClickHandler = (userId) => {
    ...
    const nob = [
      ...devolperData,
      (devolperData[positionOfTheUser].projectStatus = 3),
    ];
    setDevoloperData(nob);
  };
  return (
      ...
      <Router>
        ...
        <Switch>
          <Route
            ...
            render={() => (
              <AdminPortal
                devolperData={devolperData}
                setProjectClickHandler={setProjectClickHandler}
                endProjectClickHandler={endProjectClickHandler}
              />
            )}
          />
          <Route
            exact
            path="/devoloper/:userId"
            render={() => (
              <DevoloperPortal
                devolperData={devolperData}
                setProjectClickHandler={setProjectClickHandler}
